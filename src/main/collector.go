                        ...
                        for _, podName := range podNames {
                                ...
                                for _, cs := range pObj.Status.ContainerStatuses {
                                        ...
                                        if cs.State.Running != nil {
                                                ...
                                                containerCount++
                                                logger.Trace("找到运行中容器",
                                                        zap.String("container", cs.Name))
                                                break
                                        }
                                        logger.Trace("容器非运行状态",
                                                zap.String("container", cs.Name),
                                                zap.Any("state", cs.State))
                                }

                                if containerName == "" {
                                        logger.Info("无可用容器，跳过",
                                                zap.String("namespace", nsName),
                                                zap.String("pod", podName),
...
func (collector *TCPQueueCollector) collectSingleTask(t podTask, metricChan chan<- prometheus.Metric, cm *ConcurrencyManager) {
        ...
        // 并发控制日志
        logger.Trace("获取全局并发槽位")
        cm.AcquireGlobal()
        defer func() {
                logger.Trace("释放全局并发槽位")
                cm.ReleaseGlobal()
        }()

        logger.Trace("获取部署级并发槽位",
...
func (collector *TCPQueueCollector) Collect(metricChan chan<- prometheus.Metric) {
        logger.Info("开始收集 TCP 队列指标")
        snap, err := LoadSnapshot(collector.cacheFilePath)
        if err != nil {
                logger.Error("加载快照失败，跳过本次采集",
        ...
        tasks = sm.ShardTasks(tasks, replicas, ordinal)
