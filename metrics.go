...
type parseState struct {
	...
	tcpExtValues      map[string]float64   // TCP扩展指标值
	...
	logger            *zap.Logger         // 使用标准zap.Logger类型
	debugEnabled      bool                 // 是否启用调试日志
	expectingTCPExtData bool               // 是否期待TCPExt数据行
}

// 创建解析状态
func newParseState(logger *zap.Logger) *parseState {
	return &parseState{
		tcpExtValues:      make(map[string]float64),
		logger:            logger,
		debugEnabled:      logger.Core().Enabled(zap.DebugLevel),
		expectingTCPExtData: false,
	}
}
...
func parseMetricValue(valueStr string, logger *zap.Logger) float64 {
...
func handleTCPExtLine(lineBytes []byte, state *parseState) {
	...
	if len(fields) > 1 {
		...
		state.expectingTCPExtData = true // 设置标志，下一行将是数据行
...
// 处理TCP扩展指标数据行
func handleTCPExtDataLine(lineBytes []byte, state *parseState) {
	fields := bytes.Fields(lineBytes)
	if len(fields) < 2 {
		return
	}
	...
	for i := 1; i < len(fields); i++ {
		if i-1 >= len(state.tcpExtKeys) {
			break
		}
		
		key := state.tcpExtKeys[i-1]
		valueStr := string(fields[i])
		value := parseMetricValue(valueStr, state.logger)
		
		// 只存储我们关心的指标
		if key == KeySyncookiesSent || key == KeyListenOverflows || key == KeyListenDrops {
			state.tcpExtValues[key] = value
			if state.debugEnabled {
				state.logger.Debug("解析TCP扩展指标值",
					zap.String("key", key),
...
func streamParseAndReport(r io.Reader, collector *TCPQueueCollector,
	...
	for {
		...
		// 处理TCP扩展指标数据行（如果处于期待状态）
		if state.expectingTCPExtData {
			state.expectingTCPExtData = false
			handleTCPExtDataLine(lineBytes, state)
			handledCount++
			continue
		}
	...
	// 从解析状态中提取TCP扩展指标值
	if val, ok := state.tcpExtValues[KeySyncookiesSent]; ok {
		state.syncookiesSent = val
	}
	if val, ok := state.tcpExtValues[KeyListenOverflows]; ok {
		state.listenOverflows = val
	}
	if val, ok := state.tcpExtValues[KeyListenDrops]; ok {
		state.listenDrops = val
	}
