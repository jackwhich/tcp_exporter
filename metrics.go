...
import (
	...
	"go.uber.org/zap"
	"tcp-exporter/pkg/utils" // 使用正确的模块路径
)
...
// 创建解析状态
func newParseState(log *utils.Logger) *parseState {
	return &parseState{
		logger:       log,
...
func parseMetricValue(valueStr string, log *utils.Logger) float64 {
...
func streamParseAndReport(r io.Reader, collector *TCPQueueCollector,
	metricChan chan<- prometheus.Metric, logger *utils.Logger, namespace, pod, ip, container string) {
