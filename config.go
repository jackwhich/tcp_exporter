...
type K8sConfig struct {
	...
	TotalShards      int      `yaml:"total_shards"`        // 总分片数量
}

// CoordinatorConfig 包含协调器服务的配置
type CoordinatorConfig struct {
	Port string `yaml:"port"` // 协调器服务监听端口
}
...
type Config struct {
	...
	Coordinator CoordinatorConfig `yaml:"coordinator"` // 协调器服务配置
}
...
func mustLoadConfig() (*Config, string) {
	...
	fmt.Fprintf(os.Stdout, "配置摘要:\n")
	fmt.Fprintf(os.Stdout, "  Kubernetes:\n")
	fmt.Fprintf(os.Stdout, "    UseInCluster: %v\n", cfg.Kubernetes.UseInCluster)
	fmt.Fprintf(os.Stdout, "    KubeConfigPath: %s\n", cfg.Kubernetes.KubeConfigPath)
	fmt.Fprintf(os.Stdout, "    TargetNamespaces: %v\n", cfg.Kubernetes.TargetNamespaces)
	fmt.Fprintf(os.Stdout, "    MaxConcurrent: %d\n", cfg.Kubernetes.MaxConcurrent)
	fmt.Fprintf(os.Stdout, "    MaxPodContainer: %d\n", cfg.Kubernetes.MaxPodContainer)
	fmt.Fprintf(os.Stdout, "  Server:\n")
	fmt.Fprintf(os.Stdout, "    Port: %s\n", cfg.Server.Port)
	fmt.Fprintf(os.Stdout, "    LogLevel: %s\n", cfg.Server.LogLevel)
	fmt.Fprintf(os.Stdout, "  Coordinator:\n")
	fmt.Fprintf(os.Stdout, "    Port: %s\n", cfg.Coordinator.Port)
	
	return cfg, *configPath
}

func watchConfig(logger *zap.Logger, path string, onChange func(*Config)) {
